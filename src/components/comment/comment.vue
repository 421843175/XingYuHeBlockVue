    ...
    methods: {
      ...
      getComments(pagination, floorComment = {}, isToPage = false) {
        this.$http.post(this.$constant.baseURL + "/comment/listComment", pagination)
          .then((res) => {
            if (!this.$common.isEmpty(res.data.data) && !this.$common.isEmpty(res.data.data.records)) {
              if (this.$common.isEmpty(floorComment)) {
                ...
                if (isToPage === false) {
                  floorComment.childComments = res.data.data;
                } else {
                  floorComment.childComments.total = res.data.data.total;
                  floorComment.childComments.records = floorComment.childComments.records.concat(res.data.data.records);
                }
